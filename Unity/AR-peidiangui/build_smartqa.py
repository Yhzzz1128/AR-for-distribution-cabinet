#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Build SmartQA knowledge DOCX using compact_reference_guide preset."""
...
def sf(run, name="Calibri", size=11, bold=False, italic=False, color=None):
    ...
    if rf is None:
        rf = parse_xml(
            '<w:rFonts %s w:ascii="%s" w:hAnsi="%s" w:eastAsia="%s"/>'
            % (nsdecls("w"), name, name, name)
        )
        rPr.append(rf)
    else:
...
def scm(cell, top=80, bottom=80, start=120, end=120):
    ...
    xml = (
        ...
        '</w:tcMar>'
    ) % (nsdecls("w"), top, bottom, start, end)
...
def scw(cell, w):
    ...
    for ex in list(tcPr):
        if ex.tag == qn("w:tcW"):
            tcPr.remove(ex)
    tcPr.append(
        parse_xml('<w:tcW %s w:w="%d" w:type="dxa"/>' % (nsdecls("w"), w))
    )
...
def sti(table, indent):
    ...
    for ex in list(tblPr):
        if ex.tag == qn("w:tblInd"):
            tblPr.remove(ex)
    tblPr.append(
        parse_xml('<w:tblInd %s w:w="%d" w:type="dxa"/>' % (nsdecls("w"), indent))
    )
...
def stw(table, w):
    ...
    for ex in list(tblPr):
        if ex.tag == qn("w:tblW"):
            tblPr.remove(ex)
    tblPr.append(
        parse_xml('<w:tblW %s w:w="%d" w:type="dxa"/>' % (nsdecls("w"), w))
    )
...
def ab(table):
    ...
    b = parse_xml(
        '<w:tblBorders %s>'
        '<w:top w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
        '<w:left w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
        '<w:bottom w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
        '<w:right w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
        '<w:insideH w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
        '<w:insideV w:val="single" w:sz="4" w:space="0" w:color="C0C4CC"/>'
...
def sh(cell, hex_color):
    ...
    tcPr = tc.get_or_add_tcPr()
    tcPr.append(
        parse_xml('<w:shd %s w:fill="%s" w:val="clear"/>' % (nsdecls("w"), hex_color))
    )


# ── Load data ──
kb_path = r"D:\Unity\AR-peidiangui\Assets\Resources\SmartQA_Data.json"
...
mt = "\u77e5\u8bc6\u6761\u76ee\uff1a" + str(len(raw)) + " \u9879  |  \u5206\u7c7b\uff1a" + str(len(cats)) + " \u7c7b  |  2026\u5e746\u6708"
mr = mp.add_run(mt)
...
rp.paragraph_format.space_after = Pt(20)
rp._p.get_or_add_pPr().append(
    parse_xml(
        '<w:pBdr %s>'
        '<w:bottom w:val="single" w:sz="6" w:space="1" w:color="C0C4CC"/>'
        '</w:pBdr>' % nsdecls("w")
    )
)
...
ov = (
    "\u672c\u6587\u6863\u4e3a\u914d\u7535\u67dcAR\u667a\u80fd\u57f9\u8bad\u7cfb\u7edf\u4e2d SmartQA"
    "\uff08\u667a\u80fd\u95ee\u7b54\uff09\u6a21\u5757\u7684\u914d\u5957\u77e5\u8bc6\u5e93\u3002"
    "SmartQA \u8986\u76d6\u914d\u7535\u67dc\u63a7\u5236\u9762\u677f\u4e0a\u5e38\u89c1\u7684\u6307\u793a\u706f\u3001"
    "\u63a7\u5236\u6309\u94ae\u53ca\u6a21\u5f0f\u9009\u62e9\u5f00\u5173\u7684\u529f\u80fd\u8bf4\u660e\u3001"
    "\u6b63\u5e38/\u5f02\u5e38\u72b6\u6001\u5224\u65ad\u4e0e\u64cd\u4f5c\u8981\u70b9\uff0c"
    "\u5171\u6536\u5f55 " + str(len(raw)) + " \u6761\u77e5\u8bc6\u6761\u76ee\uff0c"
    "\u5206\u4e3a\u72b6\u6001\u6307\u793a\u706f\u3001\u63a7\u5236\u6309\u94ae\u3001\u6a21\u5f0f\u9009\u62e9\u4e09\u5927\u7c7b\u3002"
    "\u5b66\u5458\u53ef\u901a\u8fc7\u81ea\u7136\u8bed\u8a00\u63d0\u95ee\uff0c"
    "\u7cfb\u7edf\u81ea\u52a8\u5339\u914d\u5173\u952e\u8bcd\u5e76\u8fd4\u56de\u5bf9\u5e94\u77e5\u8bc6\u70b9\u3002"
)
p = doc.add_paragraph(ov)
ss(p, before=0, after=6, ls=1.25)
...
for i, lab in enumerate([
    ...
    cell = hdr.cells[i]
    sh(cell, P["hf"])
    scm(cell, P["ct"], P["cb"], P["cs"], P["ce"])
    p = cell.paragraphs[0]
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER if i == 0 else WD_ALIGN_PARAGRAPH.LEFT
    r = p.add_run(lab)
    sf(r, name=P["bf"], size=10, bold=True, color="#2E74B5")
...
for idx, (cn, names) in enumerate(cats):
    ...
    for i, (txt, align, bold) in enumerate([
        ...
        p.alignment = align
        r = p.add_run(txt)
        sf(r, name=P["bf"], size=10, bold=bold,
           color="#333333" if i > 0 else "#0B2545")
...
for b in bullets:
    ...
    bp.paragraph_format.first_line_indent = Inches(-0.187)
    br = bp.add_run("\u2022 " + b)
    sf(br, name=P["bf"], size=11, bold=False, color="#333333")
...
for cn, names in cats:
    ...
    for ename in names:
        ...
        kwp.paragraph_format.space_before = Pt(0)
        kwp.paragraph_format.space_after = Pt(4)
        kwp.paragraph_format.line_spacing = 1.15

        kwl = kwp.add_run("\u5173\u952e\u8bcd\uff1a")
        sf(kwl, name=P["bf"], size=9, bold=True, color="#888888")

        kwt = kwp.add_run("\u3001".join(entry["keywords"]))
        sf(kwt, name=P["bf"], size=9, bold=False, color="#888888")
        ...
        for seg in segs:
            ...
            if seg.startswith("\u3010") and seg.endswith("\u3011"):
                ...
                shp.paragraph_format.space_before = Pt(6)
                shp.paragraph_format.space_after = Pt(2)
                shp.paragraph_format.line_spacing = 1.20
                shr = shp.add_run(label)
                sf(shr, name=P["bf"], size=11, bold=True, color="#1F3A5F")
            else:
                cp = doc.add_paragraph()
                cp.paragraph_format.space_before = Pt(0)
                cp.paragraph_format.space_after = Pt(P["ba"])
                cp.paragraph_format.line_spacing = P["bl"]
                cr = cp.add_run(seg)
                sf(cr, name=P["bf"], size=P["bs"], bold=False, color="#333333")
...
r1 = fp.add_run()
sf(r1, name=P["bf"], size=9, bold=False, color="#888888")
r1._r.append(parse_xml('<w:fldChar %s w:fldCharType="begin"/>' % nsdecls("w")))

r2 = fp.add_run()
sf(r2, name=P["bf"], size=9, bold=False, color="#888888")
r2._r.append(
    parse_xml('<w:instrText %s xml:space="preserve"> PAGE </w:instrText>' % nsdecls("w"))
)

r3 = fp.add_run()
sf(r3, name=P["bf"], size=9, bold=False, color="#888888")
